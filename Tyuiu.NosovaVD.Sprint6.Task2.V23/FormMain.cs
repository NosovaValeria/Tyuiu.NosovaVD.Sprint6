...
using Tyuiu.NosovaVD.Sprint6.Task2.V23.Lib;
    ...
    public partial class FormMain : Form
    {
        ...
        private void buttonDone_NVD_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.chartFunction_NVD.Titles.Add("График функции");
                this.chartFunction_NVD.ChartAreas[0].AxisX.Title = "Ось X";
                this.chartFunction_NVD.ChartAreas[0].AxisY.Title = "Ось Y";
                for (int i = 0; i <= len - 1; i++)
                {
                    this.dataGridViewFunction_NVD.Rows.Add(Convert.ToString(startStep), Convert.ToString(valueArray[i]));
